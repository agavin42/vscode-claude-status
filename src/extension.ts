...
class ClaudeTerminalsProvider implements vscode.TreeDataProvider<TrackedTerminal>, vscode.TreeDragAndDropController<TrackedTerminal> {
    private _onDidChangeTreeData = new vscode.EventEmitter<TrackedTerminal | undefined>();
    ...
    handleDrag(source: readonly TrackedTerminal[], dataTransfer: vscode.DataTransfer): void {
        dataTransfer.set('application/vnd.code.tree.claudeterminals',
            new vscode.DataTransferItem(source.map(t => t.ccId)));
    }

    handleDrop(target: TrackedTerminal | undefined, dataTransfer: vscode.DataTransfer): void {
        const transferItem = dataTransfer.get('application/vnd.code.tree.claudeterminals');
    ...
    getTreeItem(terminal: TrackedTerminal): vscode.TreeItem {
        const label = terminal.customName || `Claude ${terminal.ccId.slice(-6)}`;
        ...
        switch (terminal.state) {
            ...
            case ClaudeState.Busy:
                item.iconPath = new vscode.ThemeIcon('sync~spin', new vscode.ThemeColor('warningForeground'));
                statePrefix = '🟡 BUSY';
                break;
            case ClaudeState.Waiting:
                item.iconPath = new vscode.ThemeIcon('comment-discussion', new vscode.ThemeColor('notificationsInfoIcon.foreground'));
                statePrefix = '🔵 WAITING';
                break;
            case ClaudeState.Idle:
                item.iconPath = new vscode.ThemeIcon('circle-outline', new vscode.ThemeColor('testing.iconPassed'));
                statePrefix = '🟢 idle';
                break;
            default:
        ...
        const promptPreview = terminal.lastPrompt
            ? terminal.lastPrompt.slice(0, 50) + (terminal.lastPrompt.length > 50 ? '...' : '')
            : '';
        ...
        if (terminal.lastPrompt) {
            item.tooltip.appendMarkdown(`**Last prompt:** ${terminal.lastPrompt}\n\n`);
        }
        ...
        item.contextValue = terminal.state === ClaudeState.Permissions ? 'terminal-perms' : 'terminal';
...
function log(msg: string) {
    const debug = vscode.workspace.getConfiguration('claudeCodeStatus').get('debug');
    if (debug) {
...
function saveState() {
    const persisted: PersistedTerminal[] = Array.from(trackedTerminals.values()).map(t => ({
        ccId: t.ccId,
...
function restoreState() {
    const persisted = extensionContext.workspaceState.get<PersistedTerminal[]>('trackedTerminals', []);
    const savedOrder = extensionContext.workspaceState.get<string[]>('terminalOrder', []);
    ...
    log(`Existing terminals: ${vscode.window.terminals.map(t => t.name).join(', ')}`);
    ...
    if (persistedMap.size > 0) {
        ...
        const unmatchedTerminals = vscode.window.terminals.filter(t =>
            t.name.startsWith('CC:') &&
            !Array.from(trackedTerminals.values()).some(tracked => tracked.terminal === t)
        );
        log(`Unmatched CC: terminals: ${unmatchedTerminals.map(t => t.name).join(', ') || 'none'}`);

        for (const terminal of unmatchedTerminals) {
            for (const [ccId, data] of persistedMap) {
                const stateFile = path.join(STATE_DIR, `${ccId}.state`);
                log(`Checking state file: ${stateFile} exists=${fs.existsSync(stateFile)}`);
                if (fs.existsSync(stateFile)) {
                    // Reset state file to IDLE - any stale BUSY from before reload is meaningless
                    try {
                        fs.writeFileSync(stateFile, 'IDLE\n');
                    ...
                    watchStateFile(tracked);
                    persistedMap.delete(ccId);
                    log(`Restored terminal ${ccId} -> "${terminal.name}" (fallback match)`);
                    break;
                }
            }
        }
...
function watchStateFile(tracked: TrackedTerminal) {
    ...
    const pollInterval = setInterval(() => {
        ...
        if (tracked.state === ClaudeState.Permissions || tracked.state === ClaudeState.Waiting) {
            const timeoutSeconds = vscode.workspace.getConfiguration('claudeCodeStatus').get<number>('permsTimeout', 60);
            if (tracked.permsEnteredAt && (now - tracked.permsEnteredAt) > timeoutSeconds * 1000) {
                const wasPerms = tracked.state === ClaudeState.Permissions;
                const newState = wasPerms ? ClaudeState.TimedOutPerms : ClaudeState.TimedOutWaiting;
                const stateFile = wasPerms ? 'TIMEDOUT-PERMS' : 'TIMEDOUT-WAITING';

                log(`${tracked.state} timeout for ${tracked.ccId} after ${timeoutSeconds}s, marking ${stateFile}`);
        ...
        if (newState !== tracked.state) {
            ...
            if (newState === ClaudeState.Permissions || newState === ClaudeState.Waiting) {
...
function createClaudeTerminal(args: string[] = []): TrackedTerminal | undefined {
    ensureStateDir();
    ...
    terminal.sendText(`claude${claudeArgs}`);
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const enabled = vscode.workspace.getConfiguration('claudeCodeStatus').get('enabled');
    if (!enabled) {
    ...
    outputChannel.appendLine('Make sure Claude Code hooks are configured in ~/.claude/settings.json');
    ...
    try {
        ...
        const tryRestore = (attempt: number) => {
            try {
                const terminalNames = vscode.window.terminals.map(t => t.name).filter(n => n);
    ...
    const showTerminalsCmd = vscode.commands.registerCommand(
        ...
        async () => {
            ...
            allTerminals.forEach((terminal) => {
                ...
                const tracked = Array.from(trackedTerminals.values()).find(t => t.terminal === terminal);
            ...
            items.sort((a, b) => {
                ...
                const aPriority = a.tracked ? STATE_CONFIG[a.tracked.state].priority : -1;
                const bPriority = b.tracked ? STATE_CONFIG[b.tracked.state].priority : -1;
        ...
        () => {
            ...
            outputChannel.appendLine(JSON.stringify({
                hooks: {
                    ...
                    PermissionRequest: [{ type: "command", command: getHookScriptPath() }],
                    Stop: [{ type: "command", command: getHookScriptPath() }],
                    UserPromptSubmit: [{ type: "command", command: getHookScriptPath() }],
...
function getHookScriptPath(): string {
    ...
    return path.join(os.homedir(), 'src', 'vscode-claude-status', 'scripts', 'cc-status-hook.sh');
}
